...
public class FilesDAO {
	...
	public int insert(FilesDTO dto) throws Exception{
		// SEQ PARENTSEQ ID ORIGINNAME FILENAME UPLOADDATE
	...
	// 파일 목록 받기
	public List<FilesDTO> getFilesBySeq(int parentSeq) throws Exception{
 ...
 // parentSeq를 이용한 게시글 삭제시 동시에 파일 전체 삭제 메소드
    public int deleteBoardFile(int parentSeq) throws Exception {
        String sql = "DELETE FROM files WHERE parentseq = ?";
        try(Connection con = this.getConnection();
				PreparedStatement stat = con.prepareStatement(sql);
			){
        stat.setInt(1, parentSeq);
        ...
        // 실제 파일 삭제
        String filePath = Statics.filePath;
        String fileName = this.getFileNameBySeq(parentSeq);
        File file = new File(filePath + "/" + fileName);
        if (file.exists()) {
            file.delete();
        }
